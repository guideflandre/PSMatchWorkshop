...
install.packages(c("xaringan", "xaringanExtra")) ## For building slides
remotes::install_github("guideflandre/PSMatchWorkshop", build_vignettes = TRUE)
```

## From docker image

```{bash installImage, eval = FALSE}
## Pull docker image (~3min)
sudo docker pull guillaumedeflandre/pasmatchworkshop
sudo docker run -p 8787:8787 -e PASSWORD=psmatch guillaumedeflandre/psmatchworkshop
```
Then, open your browser and search: `localhost:8787`. The username is `rstudio`
and the password is `psmatch`.
...
The code I will demonstrate is the same as the code in the vignette, but
without the explanations. It is available on the github page:
